...
static inline uint8_t wsan_dir_from_delta(int32_t dx, int32_t dy)
{
    ...
    if(ady >= (adx << 1)) {          /* near-vertical */
        return qy ? WSAN_DIR_N : WSAN_DIR_S;
    } else if(adx >= (ady << 1)) {   /* near-horizontal */
        return qx ? WSAN_DIR_E : WSAN_DIR_W;
    } else {                         /* diagonal */
        if(qx && qy)   return WSAN_DIR_NE;
        if(!qx && qy)  return WSAN_DIR_NW;
        if(qx && !qy)  return WSAN_DIR_SE;
        return WSAN_DIR_SW;
    }
}
