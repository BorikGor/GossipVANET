/*
 * RSU mote (WSAN):
 * - On QUERY from Mobile: ACK (unicast back to sender).
 * - On DATA from Mobile:  ACK (unicast back to sender).
 * - On EMERGENCY from Mobile: ACK + single avalanche rebroadcast.
...
/* ----- Non-blocking avalanche fan-out ----- */
typedef struct {
...
//static rsu_fanout_ctx_t rsu_data_ctx;    /* Warning of not used */
...
/*
 * send_ack_to()
 * What:     Send RSU/ACK unicast to target (OriginID of sender).
 * Methods:  Build minimal WSAN ACK; pack; simple_udp_sendto.
 * Creates:  wsan_fields_t; tx buffer; increments seq16.
 */
static void send_ack_to(const uip_ipaddr_t *dst,
                        uint32_t target_id, uint32_t ts_ms)
{
    //wsan_fields_t f = (wsan_fields_t){0};
    wsan_fields_t f;
    memset(&f, 0, sizeof(f));
...
/*
 * udp_rx_cb()
 * What:     Parse incoming frames; dedup; ACK; single avalanche EF.
 * Methods:  wsan_parse(); dedup ring; simple-udp; ctimer fan-out.
 * Creates:  wsan_fields_t h; payload view.
 */
                      ...
                      const uint8_t *data, uint16_t datalen)
{
    ...
    if(h.msg_type == WSAN_MSG_EMERGENCY) {
        /* ACK back to sender */
        send_ack_to(sender_addr, h.origin_id, h.ts_ms);
        /* Relay once (avalanche rules) so others hear it */
...
PROCESS_THREAD(rsu_process, ev, data)
{
    ...
    /* RSU is event-driven; no periodic jobs */
    while(1) {
        PROCESS_YIELD();
    }
